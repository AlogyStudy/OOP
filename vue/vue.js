...
(function (global, factory) {
  ...
  var Watcher = function () {
  	// Vue 的订阅者watcher 实现  
  	// cb 订阅者收到消息后需要执行的回调，一般来说这个回调都是需要视图指令的更新方法，从而达到视图更新。
  	// 分析 expOrFn 的get方法， 从而间接获得订阅的对象属性
  ...
  function initMixin(Vue) {
    Vue.prototype._init = function (options) {
    	// 1: 初始化环境变量，2: 处理Vue组件数据，3:解析挂载组件
    	// 观察者-消费者(订阅者) 模式来实现数据驱动视图  
    	// 通过设定对象属性的 setter/getter 方法来监听数据的变化， 
    	// 而每个属性的 setter 方法就是一个观察者， 当属性变化将会向订阅者发送消息，从而驱动视图更新。  
    	
    	// 利用 Watcher ， // https://github.com/vuejs/vue/blob/e9872271fa9b2a8bec1c42e65a2bb5c4df808eb2/src/watcher.js#L36
      var vm = this;
      // a uid
      vm._uid = uid++;
